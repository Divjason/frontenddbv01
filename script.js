// 카테고리별 제품 데이터
window.productsByCategory = {
  "스킨 / 토너": [
    {
      id: 1,
      name: "에스네이처 - 아쿠아 오아시스 토너 300ml",
      desc: "더블 기획 (토너 + 토너 + 화장솜)",
      image: "./Rectangle 9.png",
      thumbnail: "./Rectangle 9.png",
      reviewImage: "./Vector.png", // 리뷰 카드 이미지 (각 제품별로 수정 가능)
      starImage: "./star.png", // 별 이미지 (각 제품별로 수정 가능)
      discount: "63%",
      price: "19,500원",
      reviews: [
        {
          text: "내 피부 : 속건조 + 수부지 + 복합성피부 닦토든 흡토든 수분 차오름! 이건 진정 + 피지조절 + 흡수 미침! 피부 쫀득함",
          date: "2023.02.15",
          user: "솔직리뷰퀸_으아이",
          rating: 5,
        },
        {
          text: "흡토를 할 수록 피부 찹쌀떡 됩니다 그냥 민감성 수부지에 성분 예민 피부인데도 트러블이나 자극 전혀 없음",
          date: "2023.09.14",
          user: "jian*3319",
          rating: 5,
        },
        {
          text: "예민보스의 토너 정착템 흡토도 좋지만 개인적으로는 닦토로 애용중",
          date: "2022.10.31",
          user: "쨔쨔쨔",
          rating: 5,
        },
      ],
    },
    {
      id: 2,
      name: "토니모리 - 원더 세라마이드 모찌 토너",
      desc: "원더 세라마이드 모찌 토너 500ml",
      image: "./Rectangle 9-1.png",
      thumbnail: "./Rectangle 9-1.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "45%",
      price: "15,000원",
      reviews: [
        {
          text: "여름에 답답해서 이것 저것 못 바르겠는데 이거 하나만 발라서 흡수시키고 나가면 수분감은 있고 답답하지 않아서 너무 좋아요!!",
          date: "2025.07.21",
          user: "군고구마낸놔",
          rating: 5,
        },
        {
          text: "재구매템 + 인생 흡수 토너 1위 피부 장변 지켜주는 쫀쫀 토너",
          date: "2025.05.31",
          user: "타타니",
          rating: 4,
        },
        {
          text: "유명템은 이유가 있어요 역시!! 트러블 잘 나는 복합성 피부인데 얘가 속보습 채워줘서 무거운거 안발라도 돼요 ㅠㅠㅠㅠ넘좋아",
          date: "2024.02.23",
          user: "뜨아씨",
          rating: 5,
        },
      ],
    },
    {
      id: 3,
      name: "라운드랩 - 1025 독도 토너",
      desc: "1025 독도 토너 500ml",
      image: "./Rectangle 9-2.png",
      thumbnail: "./Rectangle 9-2.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "35%",
      price: "12,000원",
      reviews: [
        {
          text: "용량이 크고 순한 토너입니다! 평소에도 부담 없이 사용할 수 있어서 좋아요 ㅎㅎ",
          date: "2020.03.22",
          user: "gargar",
          rating: 5,
        },
        {
          text: "각질제거 피부에 안좋다는 얘기 들어서 안하는데 요철 98% 사라졌어요 ㅁㅊ...",
          date: "2020.01.09",
          user: "아시안이시안",
          rating: 4,
        },
        {
          text: "7년 넘게 쓰고있는 인생템❤️ 여드름 폭탄 시절부터 주사피부염으로 고생하던 때 그리고 지금까지 늘 곁을 지켜준 고마운 친구☺️",
          date: "2025.10.13",
          user: "댕댕222",
          rating: 5,
        },
      ],
    },
  ],
  "로션 / 에멀전": [
    {
      id: 4,
      name: "에스트라 - 아토베리어365 크림",
      desc: "[1+1][only화해] 아토베리어365 크림 80ml기획(본품+에센스 25ml+샤쉐6매)",
      image: "./Rectangle 9-3.png",
      thumbnail: "./Rectangle 9-3.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "25%",
      price: "49,500원",
      reviews: [
        {
          text: "바르자마자 속까지 촉촉해지고 피부가 편안해요 여름에는 리치할 수 있으니 지성피부는 양 조절 필수입니다!",
          date: "2025.06.12",
          user: "유앤미애나",
          rating: 5,
        },
        {
          text: "피부장벽 회복에 효과 좋다해서 바로 구매했어요 발림성이 진짜 좋고 저는 심지어 구순염에도 발라봤는데 구순염도 3일만에 나았어요!!",
          date: "2025.03.06",
          user: "thebest",
          rating: 5,
        },
        {
          text: "에스트라 라인 2년 넘게 사용중인데 너어어어어무 좋아요!! 순하고 발림성에 보습까지 최고에요!!!",
          date: "2025.09.02",
          user: "라이요",
          rating: 4,
        },
      ],
    },
    {
      id: 5,
      name: "비플레인 - 녹두 모공 타이트업 수딩 크림",
      desc: "[only화해] 녹두 모공 타이트업 수딩 크림 60ml+20ml 기획 (+녹두폼 20ml)",
      image: "./Rectangle 9-4.png",
      thumbnail: "./Rectangle 9-4.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "40%",
      price: "18,000원",
      reviews: [
        {
          text: "크림바르는 순간 얼굴 시원해져서 완전 좋아요!!! 열감이 확 내려갑니다 ☺️",
          date: "2025.05.26",
          user: "오늘도깡총",
          rating: 5,
        },
        {
          text: "바르자마자 쿨링감이 느껴져서 더운 날씨에 사용하기 딱 좋아요!!",
          date: "2025.07.09",
          user: "봄웜나",
          rating: 5,
        },
        {
          text: "피부 예민해서 트러블 날까 걱정했는데 이 제품은 그런 걱정 싹 사라졌습니다! 발림성도 좋고 마무리감 산뜻해서 잘 쓰고있어요!",
          date: "2025.04.19",
          user: "네넴넹넵넷",
          rating: 4,
        },
      ],
    },
    {
      id: 6,
      name: "에스네이처 - 아쿠아 스쿠알란 수분크림",
      desc: "아쿠아 스쿠알란 수분크림 80ml 더블+세럼 10ml 세트",
      image: "./Rectangle 9-5.png",
      thumbnail: "./Rectangle 9-5.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "30%",
      price: "16,000원",
      reviews: [
        {
          text: "2년째 쓰는 중인데 예민한 제 피부에 완전 딱 맞아요! 촉촉하고 수분 가득합니다!!!",
          date: "2025.05.11",
          user: "gggqqg",
          rating: 5,
        },
        {
          text: "내돈 내산 후기 : 트러블 잘 나는 편인데 유분감 하나도 없고 수분 가득한 느낌으로 촉촉합니다. 바르고 나서도 트러블 하나도 안나고 아침에 일어나도 촉촉해요!",
          date: "2020.03.19",
          user: "메론이메롱",
          rating: 5,
        },
        {
          text: "오일 들어간거에 트러블이 많이나서 조심해서 쓰는데 얘는 트러블도 안나고 다른 제품으로 넘어갔다가 다시 돌아왔습니다! 얘가 짱인듯해요!!",
          date: "2024.04.22",
          user: "코덕화덕",
          rating: 5,
        },
      ],
    },
  ],
  "세럼 / 에센스": [
    {
      id: 7,
      name: "토리든 - 다이브인 저분자 히알루론산 세럼",
      desc: "[only화해] 다이브인 저분자 히알루론산 세럼 100ml 기획 (+수딩크림 50ml)",
      image: "./Rectangle 9-6.png",
      thumbnail: "./Rectangle 9-6.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "38%",
      price: "27,000원",
      reviews: [
        {
          text: "보습력 최고! 찐덕거림 없고 기분 좋은 마무리감임! 지금보다 큰 짐승용량이 필요합니다!!",
          date: "2025.03.11",
          user: "멕킷스킨굿",
          rating: 5,
        },
        {
          text: "촉촉하긴 한데 극 건성들은 페이스 오일 한 두방울 섞어서 사용하는 거 추천!!",
          date: "2022.05.16",
          user: "나다이",
          rating: 5,
        },
        {
          text: "결론 : 속건조 잡는거 최고! 성분, 가성비 최고고 여러번 레이어링할 수록 좋음!",
          date: "2023.11.29",
          user: "오니가돼라",
          rating: 4,
        },
      ],
    },
    {
      id: 8,
      name: "웰라쥬 - 리얼 히알루로닉 블루 100 앰플",
      desc: "[only화해] 리얼 히알루로닉 블루 100 앰플 120ml기획(본품60ml+리필60ml)",
      image: "./Rectangle 9-7.png",
      thumbnail: "./Rectangle 9-7.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "35%",
      price: "28,000원",
      reviews: [
        {
          text: "저는 지복합성&예민 피부입니다 속당김 확실히 줄어드는 순한 보습앰플로 완전 추천해요!",
          date: "2025.06.01",
          user: "나나의대확행",
          rating: 5,
        },
        {
          text: "트러블 안남, 발림성 굿👍 수분+보습+탱탱의 탄력!! 손건조, 속보습 진심 다 잡음",
          date: "2024.01.24",
          user: "내가바로리뷰왕",
          rating: 5,
        },
        {
          text: "완전 민감하다면 비추 그러나 복합성, 수부지 강력추천 보습 앰플👍👍",
          date: "2023.12.22",
          user: "뿅뿅빵빵",
          rating: 4,
        },
      ],
    },
    {
      id: 9,
      name: "아누아 - PDRN 히알루론산 캡슐 100 세럼",
      desc: "[only화해] 피디알엔 캡슐100 세럼 50ml 대용량 기획(+크림10ml+사셰 6매)",
      image: "./Rectangle 9-8.png",
      thumbnail: "./Rectangle 9-8.png",
      reviewImage: "./Vector.png",
      starImage: "./star.png",
      discount: "40%",
      price: "24,000원",
      reviews: [
        {
          text: "초초초민감성 피부 PDRN 첫 도전!! = 트러블 제로 바르기 전후 차이가 진짜 크고 무조건 추천!! 속보습 유지력도 유지력이지만 얼굴에 광채가 돕니다!!!",
          date: "2024.11.05",
          user: "코기빵댕죠아",
          rating: 5,
        },
        {
          text: "광고보고 구매했는데 자극 없고 휴대용 사이즈가 맘에 들어서 샀어요! 레이어링 하니까 쫀쫀해요!",
          date: "2025.06.29",
          user: "베베99",
          rating: 4,
        },
        {
          text: "레이어링하니까 촉촉하고 좋았어요! 쫀득한 느낌이라 호불호는 갈릴 수 있을 것 같지만 저는 좋았습니다!!",
          date: "2024.08.12",
          user: "히히비",
          rating: 5,
        },
      ],
    },
  ],
};

// 기본 제품 데이터 (스킨/토너)
const productData = productsByCategory["스킨 / 토너"];

// 제품별 하트 상태 저장 (전역)
window.heartStates = {};

// 현재 카테고리의 제품 데이터 저장 (카테고리 변경 시 업데이트)
window.currentProductData = productData;

document.addEventListener("DOMContentLoaded", function () {
  // Fade-up animation trigger for index.html hero components
  const fadeUps = document.querySelectorAll(".fade-up");
  if (fadeUps.length) {
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.classList.add("is-in");
            io.unobserve(entry.target);
          }
        });
      },
      { threshold: 0.2 }
    );
    fadeUps.forEach((el) => io.observe(el));
  }

  // 네비게이션 링크
  const navLinks = document.querySelectorAll(".nav-link");
  navLinks.forEach((link) => {
    link.addEventListener("click", function (e) {
      e.preventDefault();
      navLinks.forEach((l) => l.classList.remove("active"));
      this.classList.add("active");
    });
  });

  // 피부타입 탭
  const skinTypeTabs = document.querySelectorAll(".skin-type-tab");
  skinTypeTabs.forEach((tab) => {
    tab.addEventListener("click", function () {
      skinTypeTabs.forEach((t) => t.classList.remove("active"));
      this.classList.add("active");
    });
  });

  // 언어 버튼
  const langButton = document.querySelector(".lang-button");
  if (langButton) {
    langButton.addEventListener("click", function (e) {
      e.preventDefault();
      // 언어 변경 로직
    });
  }

  // 검색 입력
  const searchLink = document.querySelector(".search-link");
  if (searchLink) {
    searchLink.addEventListener("click", function (e) {
      e.preventDefault();
      // 검색 페이지로 이동
    });
  }

  // 제품 선택 기능
  const productItems = document.querySelectorAll(".product-item");
  const selectedProductImage = document.querySelector(
    ".selected-product-image img"
  );
  const selectedProductName = document.querySelectorAll(
    ".selected-product-name div"
  );
  const selectedProductPrice = document.querySelector(
    ".selected-product-price"
  );
  const reviewsGrid = document.querySelector(".reviews-grid");
  const heartBtn = document.querySelector(".heart-btn");

  productItems.forEach((item, index) => {
    item.addEventListener("click", function () {
      // 선택 상태 업데이트
      productItems.forEach((p) => p.classList.remove("selected"));
      item.classList.add("selected");

      // 제품 정보 업데이트 (카테고리 변경 후에는 window.currentProductData 사용)
      const currentData = window.currentProductData || productData;
      const product = currentData[index];
      if (product) {
        // 이미지 변경
        if (selectedProductImage) {
          selectedProductImage.src = product.image;
        }

        // 이름 및 설명 변경
        if (selectedProductName.length >= 2) {
          selectedProductName[0].textContent = product.name;
          selectedProductName[1].textContent = product.desc;
        }

        // 가격 변경
        if (selectedProductPrice) {
          selectedProductPrice.innerHTML = `<span class="price-discount">${product.discount}</span> <span> ${product.price}</span>`;
        }

        // 리뷰 변경
        if (reviewsGrid && product.reviews) {
          reviewsGrid.innerHTML = "";
          product.reviews.forEach((review) => {
            const reviewCard = document.createElement("div");
            reviewCard.className = "review-card";
            reviewCard.innerHTML = `
                            <img src="${
                              product.reviewImage || "./Vector.png"
                            }" alt="벡터" class="Vector" style="width:18px; height:18px;">
                            <div class="review-text">${review.text}</div>
                            <div class="review-meta">
                                <img src="${
                                  product.starImage || "./star.png"
                                }" alt="">
                                <span>${review.date}</span>
                                <span>|</span>
                                <span>${review.user}</span>
                            </div>
                        `;
            reviewsGrid.appendChild(reviewCard);
          });
        }

        // 제품별 하트 상태 복원
        if (heartBtn) {
          if (window.heartStates[product.id] === true) {
            heartBtn.classList.add("active");
            const img = heartBtn.querySelector("img");
            if (img) {
              img.src = "./heart_color.png";
              img.alt = "채워진 하트";
            }
          } else {
            heartBtn.classList.remove("active");
            const img = heartBtn.querySelector("img");
            if (img) {
              img.src = "./heart.png";
              img.alt = "하트";
            }
          }
        }
      }
    });
  });

  // 카테고리 토글 드롭다운
  const categorySelector = document.querySelector(".category-selector");
  const categoryDropdown = document.querySelector(".category-dropdown");
  const categoryText = document.querySelector(".category-text");
  const categoryArrow = document.querySelector(".category-arrow");
  const categoryOptions = document.querySelectorAll(".category-option");

  if (categorySelector && categoryDropdown) {
    let isOpen = false;

    categorySelector.addEventListener("click", function (e) {
      e.stopPropagation();
      isOpen = !isOpen;
      categoryDropdown.classList.toggle("open", isOpen);
      if (categoryArrow) {
        categoryArrow.style.transform = isOpen
          ? "rotate(180deg)"
          : "rotate(0deg)";
        categoryArrow.style.transition = "transform 0.2s";
      }
    });

    // 카테고리 옵션 선택
    categoryOptions.forEach((option) => {
      option.addEventListener("click", function (e) {
        e.stopPropagation();
        const selectedCategory = this.getAttribute("data-category");
        if (!selectedCategory) return;

        // 모든 옵션에서 selected 제거
        categoryOptions.forEach((opt) => opt.classList.remove("selected"));

        // 선택한 옵션에 selected 추가
        this.classList.add("selected");

        // 텍스트 업데이트
        if (categoryText) {
          categoryText.textContent = selectedCategory;
        }

        // 드롭다운 닫기
        isOpen = false;
        categoryDropdown.classList.remove("open");
        if (categoryArrow) {
          categoryArrow.style.transform = "rotate(0deg)";
        }

        // 카테고리에 맞는 제품 데이터로 업데이트
        const newProducts = window.productsByCategory[selectedCategory] || [];
        if (newProducts.length > 0) {
          // 전역 productData 업데이트 (toggleHeart 함수에서 사용)
          if (typeof productData !== "undefined") {
            // productData는 const이므로 직접 재할당할 수 없으므로,
            // window 객체에 저장하거나 함수 내에서 동적으로 접근하도록 처리
            // 대신 전역 변수로 관리
            window.currentProductData = newProducts;
          }

          // 제품 리스트 업데이트
          const productList = document.querySelector(".product-list");
          if (productList) {
            productList.innerHTML = "";
            newProducts.forEach((product, index) => {
              const productItem = document.createElement("div");
              productItem.className =
                "product-item" + (index === 0 ? " selected" : "");
              productItem.innerHTML = `
                                <img src="${product.thumbnail}" alt="">
                                <div class="product-info">
                                    <div>${product.name}</div>
                                    <div>${product.desc}</div>
                                </div>
                            `;
              productList.appendChild(productItem);
            });

            // 첫 번째 제품을 선택된 상태로 설정하고 정보 업데이트
            const firstProduct = newProducts[0];
            updateProductInfo(firstProduct);
            updateReviews(firstProduct.reviews, firstProduct);
            updateHeartState(firstProduct);

            // 제품 선택 이벤트 다시 연결
            const newProductItems = document.querySelectorAll(".product-item");
            newProductItems.forEach((item, idx) => {
              item.addEventListener("click", function () {
                newProductItems.forEach((p) => p.classList.remove("selected"));
                item.classList.add("selected");
                const product = newProducts[idx];
                if (product) {
                  updateProductInfo(product);
                  updateReviews(product.reviews, product);
                  updateHeartState(product);
                }
              });
            });
          }
        }
      });
    });

    // 제품 정보 업데이트 함수
    function updateProductInfo(product) {
      const selectedProductImage = document.querySelector(
        ".selected-product-image img"
      );
      const selectedProductName = document.querySelectorAll(
        ".selected-product-name div"
      );
      const selectedProductPrice = document.querySelector(
        ".selected-product-price"
      );

      if (selectedProductImage) {
        selectedProductImage.src = product.image;
      }

      if (selectedProductName.length >= 2) {
        selectedProductName[0].textContent = product.name;
        selectedProductName[1].textContent = product.desc;
      }

      if (selectedProductPrice) {
        selectedProductPrice.innerHTML = `<span class="price-discount">${product.discount}</span> <span> ${product.price}</span>`;
      }
    }

    // 리뷰 업데이트 함수
    function updateReviews(reviews, product) {
      const reviewsGrid = document.querySelector(".reviews-grid");
      if (reviewsGrid && reviews) {
        reviewsGrid.innerHTML = "";
        reviews.forEach((review) => {
          const reviewCard = document.createElement("div");
          reviewCard.className = "review-card";
          reviewCard.innerHTML = `
                        <img src="${
                          product.reviewImage || "./Vector.png"
                        }" alt="벡터" class="Vector" style="width:18px; height:18px;">
                        <div class="review-text">${review.text}</div>
                        <div class="review-meta">
                            <img src="${
                              product.starImage || "./star.png"
                            }" alt="">
                            <span>${review.date}</span>
                            <span>|</span>
                            <span>${review.user}</span>
                        </div>
                    `;
          reviewsGrid.appendChild(reviewCard);
        });
      }
    }

    // 하트 상태 업데이트 함수
    function updateHeartState(product) {
      if (heartBtn) {
        if (window.heartStates[product.id] === true) {
          heartBtn.classList.add("active");
          const img = heartBtn.querySelector("img");
          if (img) {
            img.src = "./heart_color.png";
            img.alt = "채워진 하트";
          }
        } else {
          heartBtn.classList.remove("active");
          const img = heartBtn.querySelector("img");
          if (img) {
            img.src = "./heart.png";
            img.alt = "하트";
          }
        }
      }
    }

    // 외부 클릭 시 드롭다운 닫기
    document.addEventListener("click", function (e) {
      if (categorySelector && categoryDropdown && isOpen) {
        if (
          !categorySelector.contains(e.target) &&
          !categoryDropdown.contains(e.target)
        ) {
          isOpen = false;
          categoryDropdown.classList.remove("open");
          if (categoryArrow) {
            categoryArrow.style.transform = "rotate(0deg)";
          }
        }
      }
    });
  }

  // 설문조사 라디오 버튼
  const surveyItems = document.querySelectorAll(".survey-item");
  surveyItems.forEach((surveyItem) => {
    const ratingButtons = surveyItem.querySelectorAll(".rating-btn");
    ratingButtons.forEach((button) => {
      button.addEventListener("click", function () {
        // 같은 질문의 모든 버튼에서 active 제거
        ratingButtons.forEach((btn) => btn.classList.remove("active"));
        // 클릭한 버튼에 active 추가
        this.classList.add("active");

        // 선택 상태를 localStorage에 저장
        saveSurveyState(surveyItem, this);
      });
    });
  });

  // 페이지 로드 시 설문조사 선택 상태 복원
  restoreSurveyStates();

  // review-card 자동 스크롤 애니메이션
  initReviewCardAutoScroll();
});

// review-card 자동 스크롤 함수
function initReviewCardAutoScroll() {
  const reviewsGrid = document.querySelector(".reviews-grid");
  if (!reviewsGrid) return;

  const reviewCards = reviewsGrid.querySelectorAll(".review-card");
  if (reviewCards.length <= 1) return;

  // 자동 스크롤 시간 간격 설정 (밀리초) - 여기서 초 변경 가능
  const SCROLL_INTERVAL = 4000; //(원하는 초로 변경 가능)

  let currentIndex = 0;
  let autoScrollInterval = null;

  // 카드 너비 계산 (카드 + gap)
  const cardWidth = reviewCards[0].offsetWidth;
  const gap = parseInt(getComputedStyle(reviewsGrid).gap) || 15;
  const totalWidth = cardWidth + gap;

  // 자동 스크롤 함수
  function scrollToNext() {
    currentIndex = (currentIndex + 1) % reviewCards.length;
    const scrollPosition = currentIndex * totalWidth;

    reviewsGrid.scrollTo({
      left: scrollPosition,
      behavior: "smooth",
    });
  }

  // 자동 스크롤 시작
  function startAutoScroll() {
    if (autoScrollInterval) {
      clearInterval(autoScrollInterval);
    }
    autoScrollInterval = setInterval(scrollToNext, SCROLL_INTERVAL);
  }

  // 자동 스크롤 중지
  function stopAutoScroll() {
    if (autoScrollInterval) {
      clearInterval(autoScrollInterval);
      autoScrollInterval = null;
    }
  }

  // 초기 시작
  startAutoScroll();

  // 마우스 호버 시 일시정지
  reviewsGrid.addEventListener("mouseenter", stopAutoScroll);

  // 마우스 떠날 때 다시 시작
  reviewsGrid.addEventListener("mouseleave", startAutoScroll);

  // 터치 이벤트 시 일시정지
  reviewsGrid.addEventListener("touchstart", function () {
    stopAutoScroll();
  });

  reviewsGrid.addEventListener("touchend", function () {
    // 터치 종료 후 자동 스크롤 시간만큼 뒤에 다시 시작
    setTimeout(startAutoScroll, SCROLL_INTERVAL);
  });

  // 스크롤 위치를 추적하여 현재 인덱스 업데이트
  reviewsGrid.addEventListener("scroll", function () {
    const scrollLeft = reviewsGrid.scrollLeft;
    currentIndex = Math.round(scrollLeft / totalWidth);
  });
}

// 설문조사 평점 선택 함수 (onclick용)
function selectRating(button) {
  const surveyItem = button.closest(".survey-item");
  if (surveyItem) {
    const ratingButtons = surveyItem.querySelectorAll(".rating-btn");
    ratingButtons.forEach((btn) => btn.classList.remove("active"));
    button.classList.add("active");

    // 선택 상태를 localStorage에 저장
    saveSurveyState(surveyItem, button);

    // 설문조사 완료 상태 확인 (main.html의 버튼 활성화/비활성화)
    if (typeof checkSurveyCompletion === "function") {
      setTimeout(checkSurveyCompletion, 100);
    }

    // 활성화 확인을 위한 콘솔 로그 (디버깅용)
    console.log("Rating button activated:", button);
  }
}

// 설문조사 선택 상태를 localStorage에 저장
function saveSurveyState(surveyItem, selectedButton) {
  try {
    // survey-item의 클래스에서 질문 번호 찾기 (survey-item-1, survey-item-2, survey-item-3 등)
    let questionId = null;
    const classList = surveyItem.classList;
    for (let i = 0; i < classList.length; i++) {
      const className = classList[i];
      if (className.startsWith("survey-item-")) {
        questionId = className;
        break;
      }
    }

    if (!questionId) {
      // survey-item-1, survey-item-2 형식이 없으면 인덱스 사용
      const allSurveyItems = document.querySelectorAll(".survey-item");
      const index = Array.from(allSurveyItems).indexOf(surveyItem);
      if (index >= 0) {
        questionId = `survey-item-${index + 1}`;
      }
    }

    if (!questionId) return;

    // 선택한 버튼의 번호 찾기
    const numberSpan = selectedButton.querySelector(".number");
    const ratingNumber = numberSpan ? numberSpan.textContent.trim() : null;

    if (ratingNumber) {
      // localStorage에 저장: { questionId: ratingNumber }
      const surveyStates = getSurveyStates();
      surveyStates[questionId] = ratingNumber;
      localStorage.setItem("surveyStates", JSON.stringify(surveyStates));
    }
  } catch (e) {
    console.error("설문조사 상태 저장 실패:", e);
  }
}

// localStorage에서 설문조사 선택 상태 가져오기
function getSurveyStates() {
  try {
    const stored = localStorage.getItem("surveyStates");
    return stored ? JSON.parse(stored) : {};
  } catch (e) {
    console.error("설문조사 상태 불러오기 실패:", e);
    return {};
  }
}

// 페이지 로드 시 설문조사 선택 상태 복원
function restoreSurveyStates() {
  try {
    const surveyStates = getSurveyStates();
    const surveyItems = document.querySelectorAll(".survey-item");

    surveyItems.forEach((surveyItem) => {
      // survey-item의 클래스에서 질문 번호 찾기
      let questionId = null;
      const classList = surveyItem.classList;
      for (let i = 0; i < classList.length; i++) {
        const className = classList[i];
        if (className.startsWith("survey-item-")) {
          questionId = className;
          break;
        }
      }

      if (!questionId) {
        // survey-item-1, survey-item-2 형식이 없으면 인덱스 사용
        const allSurveyItems = document.querySelectorAll(".survey-item");
        const index = Array.from(allSurveyItems).indexOf(surveyItem);
        if (index >= 0) {
          questionId = `survey-item-${index + 1}`;
        }
      }

      if (!questionId || !surveyStates[questionId]) return;

      // 저장된 선택 상태 복원
      const savedRating = surveyStates[questionId];
      const ratingButtons = surveyItem.querySelectorAll(".rating-btn");
      ratingButtons.forEach((button) => {
        const numberSpan = button.querySelector(".number");
        if (numberSpan && numberSpan.textContent.trim() === savedRating) {
          // 같은 질문의 모든 버튼에서 active 제거
          ratingButtons.forEach((btn) => btn.classList.remove("active"));
          // 저장된 버튼에 active 추가
          button.classList.add("active");
        }
      });
    });
  } catch (e) {
    console.error("설문조사 상태 복원 실패:", e);
  }
}

// 하트 토글 함수 (onclick용, HTML에서 직접 호출 가능)
function toggleHeart(btn) {
  const img = btn.querySelector("img");

  // 현재 선택된 제품 찾기
  const productItems = document.querySelectorAll(".product-item");
  let currentProductIndex = -1;
  productItems.forEach((item, index) => {
    if (item.classList.contains("selected")) {
      currentProductIndex = index;
    }
  });

  // 제품 데이터에서 ID 가져오기 (카테고리 변경 후에도 작동하도록)
  let productId = null;
  let currentProductData = null;

  // window.currentProductData를 먼저 확인 (카테고리 변경 후)
  if (
    window.currentProductData &&
    window.currentProductData[currentProductIndex]
  ) {
    currentProductData = window.currentProductData;
  } else if (
    typeof productData !== "undefined" &&
    productData[currentProductIndex]
  ) {
    // 기본 productData 확인
    currentProductData = productData;
  }

  if (currentProductData && currentProductData[currentProductIndex]) {
    productId = currentProductData[currentProductIndex].id;
  } else if (currentProductIndex >= 0) {
    // 제품 데이터를 찾을 수 없으면 인덱스 사용
    productId = currentProductIndex;
  }

  if (productId === null) {
    console.log("제품 ID를 찾을 수 없습니다.");
    return;
  }

  if (btn.classList.contains("active")) {
    btn.classList.remove("active");
    img.src = "./heart.png";
    img.alt = "하트";
    window.heartStates[productId] = false;
  } else {
    btn.classList.add("active");
    img.src = "./heart_color.png";
    img.alt = "채워진 하트";
    window.heartStates[productId] = true;
  }
}

/* =========================================================
   [signup_info 이식] page1-info.html의 수집·전송 로직
   - 세션 생성(/api/session) → /api/profile 저장 → signup_choice.html 이동
   - 커스텀 생년월일 드롭다운 → hidden(year/month/day) 동기화
   - 성별 버튼 → hidden(gender) 동기화
   ========================================================= */
(function () {
  const API_BASE = (
    localStorage.getItem("API_BASE") || "http://localhost:12345"
  ).replace(/\/+$/, "");

  // --- 세션 관리 (page1-info.html의 동작을 동일 재현) ---
  function getSessionId() {
    let sid = localStorage.getItem("faceapp_session");
    if (!sid) {
      sid =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : String(Date.now()) + Math.random().toString(16).slice(2);
      localStorage.setItem("faceapp_session", sid);
      // 서버에도 등록 (실패해도 진행)
      const fd = new FormData();
      fd.append("session_id", sid);
      fetch(`${API_BASE}/api/session`, { method: "POST", body: fd }).catch(
        () => {}
      );
    }
    return sid;
  }

  function startNewSession() {
    // 이전 세션 및 프로필 캐시 제거 후 새 sid 발급
    localStorage.removeItem("faceapp_session");
    localStorage.removeItem("faceapp_profile");
    getSessionId();
  }

  // --- 커스텀 드롭다운 조립: hidden(year/month/day) 동기화 ---
  function initBirthDropdowns(form) {
    const yearBox = form.querySelector("#year-dropdown");
    const monthBox = form.querySelector("#month-dropdown");
    const dayBox = form.querySelector("#day-dropdown");

    const yearLabel = form.querySelector(
      ".date-input:not(.month):not(.day) .date-value"
    );
    const monthLabel = form.querySelector(".date-input.month .date-value");
    const dayLabel = form.querySelector(".date-input.day .date-value");

    const hiddenYear = form.querySelector("#birth_year");
    const hiddenMonth = form.querySelector("#birth_month");
    const hiddenDay = form.querySelector("#birth_day");

    if (
      !yearBox ||
      !monthBox ||
      !dayBox ||
      !hiddenYear ||
      !hiddenMonth ||
      !hiddenDay
    )
      return;

    function daysIn(y, m) {
      return new Date(y, m, 0).getDate();
    }
    function fill(box, arr, onPick) {
      box.innerHTML = "";
      arr.forEach((v) => {
        const el = document.createElement("div");
        el.className = "date-option";
        el.textContent =
          (typeof v === "number" ? v : String(v)) +
          (box === yearBox ? "년" : box === monthBox ? "월" : "일");
        el.addEventListener("click", () => onPick(v));
        box.appendChild(el);
      });
    }

    // 외부에서 호출되는 토글 핸들러 (HTML에 그대로 유지된 onclick과 호환)
    window.toggleDropdown = function (wrapper, type) {
      const dd = wrapper.nextElementSibling;
      const willOpen = !dd.classList.contains("open");
      document
        .querySelectorAll(".date-dropdown.open")
        .forEach((x) => x.classList.remove("open"));
      if (!willOpen) return;
      dd.classList.add("open");

      if (type === "year" && !yearBox.children.length) {
        const now = new Date(),
          thisYear = now.getFullYear();
        const years = Array.from(
          { length: thisYear - 1940 + 1 },
          (_, i) => thisYear - i
        );
        fill(yearBox, years, pickYear);
      }
      if (type === "month" && !monthBox.children.length) {
        fill(
          monthBox,
          Array.from({ length: 12 }, (_, i) => i + 1),
          pickMonth
        );
      }
      if (type === "day") {
        const y = Number(hiddenYear.value) || new Date().getFullYear();
        const m = Number(hiddenMonth.value) || new Date().getMonth() + 1;
        fill(
          dayBox,
          Array.from({ length: daysIn(y, m) }, (_, i) => i + 1),
          pickDay
        );
      }
    };

    function pickYear(y) {
      hiddenYear.value = String(y);
      yearLabel.textContent = `${y}년`;
      yearLabel.parentElement.classList.add("selected");
      document
        .querySelectorAll(".date-dropdown.open")
        .forEach((x) => x.classList.remove("open"));
      // 월/일 의존성 처리
      if (hiddenMonth.value) {
        const dmax = daysIn(
          Number(hiddenYear.value),
          Number(hiddenMonth.value)
        );
        if (!hiddenDay.value || Number(hiddenDay.value) > dmax) {
          hiddenDay.value = "";
          dayLabel.textContent = "일";
          dayLabel.parentElement.classList.remove("selected");
        }
        // day 리스트 갱신은 다음 open 시 자동
      }
    }
    function pickMonth(m) {
      hiddenMonth.value = String(m).padStart(2, "0");
      monthLabel.textContent = `${m}월`;
      monthLabel.parentElement.classList.add("selected");
      document
        .querySelectorAll(".date-dropdown.open")
        .forEach((x) => x.classList.remove("open"));
      const y = Number(hiddenYear.value) || new Date().getFullYear();
      const dmax = daysIn(y, Number(m));
      if (!hiddenDay.value || Number(hiddenDay.value) > dmax) {
        hiddenDay.value = "";
        dayLabel.textContent = "일";
        dayLabel.parentElement.classList.remove("selected");
      }
    }
    function pickDay(d) {
      hiddenDay.value = String(d).padStart(2, "0");
      dayLabel.textContent = `${d}일`;
      dayLabel.parentElement.classList.add("selected");
      document
        .querySelectorAll(".date-dropdown.open")
        .forEach((x) => x.classList.remove("open"));
    }

    // 폼 밖 클릭 시 닫기
    document.addEventListener("click", (e) => {
      if (!form.contains(e.target)) {
        document
          .querySelectorAll(".date-dropdown.open")
          .forEach((x) => x.classList.remove("open"));
      }
    });
  }

  // --- 성별 버튼 → hidden(gender) 동기화 ---
  function initGenderButtons(form) {
    const box = form.querySelector("#genderButtons");
    const hidden = form.querySelector("#gender");
    if (!box || !hidden) return;

    box.addEventListener("click", (e) => {
      const btn = e.target.closest(".gender-btn");
      if (!btn) return;
      box
        .querySelectorAll(".gender-btn")
        .forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      hidden.value = btn.dataset.value || "";
    });
  }

  // --- 제출 처리: /api/profile 저장 후 다음 페이지 이동 ---
  async function handleSubmit(e) {
    e.preventDefault();
    const form = e.currentTarget;

    const nickname = form.querySelector('[name="nickname"]')?.value?.trim();
    const yy = form.querySelector("#birth_year")?.value;
    const mm = form.querySelector("#birth_month")?.value;
    const dd = form.querySelector("#birth_day")?.value;
    const gender = form.querySelector("#gender")?.value;

    if (!nickname) return alert("닉네임을 입력해주세요.");
    if (!yy || !mm || !dd) return alert("생년월일을 선택해주세요.");
    if (!gender) return alert("성별을 선택해주세요.");

    // page1-info.html과 동일: 세션 시작 및 서버 등록 → 프로필 저장
    const fd = new FormData();
    fd.append("session_id", getSessionId());
    fd.append("nickname", nickname);
    fd.append("birth_date", `${yy}-${mm}-${dd}`);
    fd.append("gender", gender);

    const r = await fetch(`${API_BASE}/api/profile`, {
      method: "POST",
      body: fd,
    });

    // /api/profile POST 성공 직후에 추가
    const profile = JSON.parse(localStorage.getItem("faceapp_profile") || "{}");
    profile.nickname = nickname;
    profile.birth_date = `${yy}-${mm}-${dd}`; // 원문 그대로 보관
    profile.gender = gender;
    profile.timestamp_info = new Date().toISOString(); // 수집시각
    localStorage.setItem("faceapp_profile", JSON.stringify(profile));

    if (!r.ok) {
      const t = await r.text();
      console.error(t);
      return alert("정보 저장에 실패했습니다.");
    }

    // 다음 페이지로 이동 (기존 흐름과 동일)
    location.href = "signup_choice.html";
  }

  // --- 페이지 바인딩 (signup_info 전용) ---
  document.addEventListener("DOMContentLoaded", () => {
    if (document.body.dataset.page !== "signup_info") return;

    // page1의 UX 그대로: 진입 시 새 세션 시작(세션 단위 누적)  :contentReference[oaicite:2]{index=2}
    startNewSession();

    const form = document.querySelector("#infoForm");
    if (!form) return;
    initBirthDropdowns(form);
    initGenderButtons(form);

    form.addEventListener("submit", handleSubmit);
  });
})();

/* =========================================================
   [signup_choice 이식] page2-skin.html의 수집·전송 로직
   - 세션 생성(/api/session) → /api/skin 저장 → ai_test_camera.html 이동
   - 선택 UI는 signup_choice.html의 .skin-btn.active 상태를 그대로 활용
   ========================================================= */
(function () {
  // 필요에 따라 localhost:12345를 환경에 맞춰 바꾸세요.
  const API_BASE = (
    localStorage.getItem("API_BASE") || "http://localhost:12345"
  ).replace(/\/+$/, "");
  const NEXT_PAGE = "ai_test_camera.html"; // 저장 후 이동할 페이지

  // --- page2-skin.html과 동일한 세션 발급/등록 흐름 ---  :contentReference[oaicite:5]{index=5}
  function getSessionId() {
    let sid = localStorage.getItem("faceapp_session");
    if (!sid) {
      sid =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : String(Date.now()) + Math.random().toString(16).slice(2);
      localStorage.setItem("faceapp_session", sid);
      // 서버에도 등록 (실패해도 진행)
      const fd = new FormData();
      fd.append("session_id", sid);
      fetch(`${API_BASE}/api/session`, { method: "POST", body: fd }).catch(
        () => {}
      );
    }
    return sid;
  }

  // DOM 헬퍼
  const $ = (s, el = document) => el.querySelector(s);
  const $$ = (s, el = document) => [...el.querySelectorAll(s)];

  // 현재 페이지가 signup_choice가 아니면 무시
  document.addEventListener("DOMContentLoaded", () => {
    const isChoice =
      document.body &&
      document.body.dataset &&
      document.body.dataset.page === "signup_choice";
    if (!isChoice) return;

    const startBtn =
      document.getElementById("start-button") ||
      document.getElementById("start");
    if (!startBtn) return;

    // 클릭 시: 선택값 수집 → 서버로 전송 → 다음 페이지
    // startBtn.addEventListener("click", async (e) => {
    //   // inline onClick(예: validateSurvey())와 함께 동작해도 문제 없도록 기본 동작만 차단
    //   e.preventDefault();

    //   try {
    //     // 1) 선택값 읽기 (UI 클래스/구조는 signup_choice.html 것을 그대로 사용)  :contentReference[oaicite:6]{index=6}
    //     const typeBtn = $(".skin-type-section .skin-btn.active");
    //     const skinType = typeBtn ? typeBtn.textContent.trim() : null;

    //     const concernBtns = $$(".skin-concern-section .skin-btn.active");
    //     const concerns = concernBtns.map((b) => b.textContent.trim());

    //     const conditionBtns = $$(".skin-condition-section .skin-btn.active");
    //     const conditions = conditionBtns.map((b) => b.textContent.trim());

    //     if (!skinType) {
    //       alert("피부 타입을 선택해주세요.");
    //       return;
    //     }

    //     // 2) 로컬에도 캐시 (page2-skin.html과 동일 컨벤션 사용: faceapp_profile)  :contentReference[oaicite:7]{index=7}
    //     const profile = JSON.parse(
    //       localStorage.getItem("faceapp_profile") || "{}"
    //     );
    //     profile.skinType = skinType;
    //     profile.concerns = concerns;
    //     profile.conditions = conditions;
    //     localStorage.setItem("faceapp_profile", JSON.stringify(profile));

    //     // 3) 서버 저장 (page2-skin.html과 동일: FormData로 /api/skin POST)  :contentReference[oaicite:8]{index=8}
    //     const fd = new FormData();
    //     fd.append("session_id", getSessionId());
    //     fd.append("skin_type", skinType);
    //     fd.append("concerns", JSON.stringify(concerns));
    //     fd.append("conditions", JSON.stringify(conditions));

    //     const resp = await fetch(`${API_BASE}/api/skin`, {
    //       method: "POST",
    //       body: fd,
    //     });
    //     if (!resp.ok) {
    //       const txt = await resp.text();
    //       console.error(txt);
    //       alert("피부 정보 저장에 실패했습니다.");
    //       return;
    //     }

    //     // 4) 다음 페이지로 이동 (기존 흐름에 맞게 수정 가능)
    //     location.href = NEXT_PAGE;
    //   } catch (err) {
    //     console.error(err);
    //     alert("처리 중 오류가 발생했습니다.");
    //   }
    // });
  });
})();

// ===================== 공통 설정 =====================
const API_BASE = (
  localStorage.getItem("API_BASE") || "http://localhost:12345"
).replace(/\/+$/, "");

// page1/page2와 동일: 세션 ID 로컬+서버 등록  :contentReference[oaicite:9]{index=9}
function getSessionId() {
  let sid = localStorage.getItem("faceapp_session");
  if (!sid) {
    sid =
      crypto && crypto.randomUUID
        ? crypto.randomUUID()
        : String(Date.now()) + Math.random().toString(16).slice(2);
    localStorage.setItem("faceapp_session", sid);
    const fd = new FormData();
    fd.append("session_id", sid);
    fetch(`${API_BASE}/api/session`, { method: "POST", body: fd }).catch(
      () => {}
    );
  }
  return sid;
}

/***********************
 *  UTIL
 ***********************/
function q(sel) {
  return document.querySelector(sel);
}
function qa(sel) {
  return Array.from(document.querySelectorAll(sel));
}
function firstSel(selectors) {
  for (const s of selectors) {
    const el = typeof s === "string" ? q(s) : s;
    if (el) return el;
  }
  return null;
}

// 세션 생성/등록 (index/script_1와 동일 컨벤션)
function getSessionId() {
  let sid = localStorage.getItem("faceapp_session");
  if (!sid) {
    sid =
      crypto && crypto.randomUUID
        ? crypto.randomUUID()
        : String(Date.now()) + Math.random().toString(16).slice(2);
    localStorage.setItem("faceapp_session", sid);
    // 서버에도 등록 (실패해도 진행)
    const fd = new FormData();
    fd.append("session_id", sid);
    fetch(`${API_BASE}/api/session`, { method: "POST", body: fd }).catch(
      () => {}
    );
  }
  return sid;
}

// 업로드 (index.html + script_1.js의 전송 규격 그대로)
// FormData: image, gender, age, session_id
async function uploadCapture(blob, gender, age) {
  const fd = new FormData();
  // fd.append("image", blob, `face_${Date.now()}.jpg`);
  fd.append("image", blob, `face_${Date.now()}.jpg`);
  // fd.append("file", blob, `face_${Date.now()}.jpg`);
  fd.append("gender", gender || "");
  // fd.append("age", age || "");
  // ✅ 나이 → 버킷 계산 (20/30/40/50/60, 미만은 10으로)
  const profile = JSON.parse(localStorage.getItem("faceapp_profile") || "{}");
  let ageNum = Number(age);
  if (!ageNum && profile.birth_date) {
    const by = new Date(profile.birth_date).getFullYear();
    ageNum = new Date().getFullYear() - by;
  }
  if (!ageNum) {
    const shootAge = Number(localStorage.getItem("shoot_age") || "");
    if (shootAge) ageNum = shootAge;
  }
  // 버킷 규칙
  let bucket = null;
  if (ageNum) {
    if (ageNum >= 60) bucket = 60;
    else if (ageNum >= 50) bucket = 50;
    else if (ageNum >= 40) bucket = 40;
    else if (ageNum >= 30) bucket = 30;
    else if (ageNum >= 20) bucket = 20;
    else bucket = 10; // 10대 이하
  }
  // if (bucket) fd.append("age_bucket", String(bucket));
  if (bucket) {
    fd.append("age_bucket", String(bucket)); // 서버가 age_bucket을 기대하는 경우
    fd.append("age", String(bucket)); // 서버가 age(정수)를 기대하는 경우
  }

  // 동의 여부 전달(동의 페이지에서 저장해둔 값 활용)
  // fd.append("consent", profile.consent ? "1" : "0");
  fd.append("consent", profile.consent ? "1" : "0");
  fd.append("session_id", getSessionId());

  const res = await fetch(`${API_BASE}/api/captures`, {
    method: "POST",
    body: fd,
  });
  // let data = null;
  // try {
  //   data = await res.json();
  // } catch (e) {}
  // if (!res.ok || (data && data.ok === false)) {
  //   throw new Error((data && data.error) || `Upload failed ${res.status}`);
  // }
  // // { ok, id, image_path, image_url } 형식 기대
  // return data;
  // if (!res.ok) {
  //   let msg = `Upload failed ${res.status}`;
  //   try {
  //     const t = await res.text();
  //     if (t) msg += `: ${t}`;
  //   } catch {}
  //   throw new Error(msg);
  // }
  if (!res.ok) {
    let msg = `Upload failed ${res.status}`;
    try {
      msg += `: ${(await res.text()) || ""}`;
    } catch {}
    throw new Error(msg);
  }
  const ct = res.headers.get("content-type") || "";
  if (ct.includes("application/json")) {
    try {
      const data = await res.json();
      return data && typeof data === "object" ? data : {};
    } catch {
      return {};
    }
  } else {
    const url = (await res.text()).trim();
    return url ? { image_url: url } : {};
  }
  // 응답이 JSON이면 파싱, 아니면 text를 URL로 가정
  // const ct = res.headers.get("content-type") || "";
  // if (ct.includes("application/json")) {
  //   try {
  //     const data = await res.json();
  //     // 항상 객체 반환
  //     return typeof data === "object" && data !== null ? data : {};
  //   } catch {
  //     return {};
  //   }
  // } else {
  //   // 서버가 URL 문자열만 돌려줄 때 대응
  //   const url = (await res.text()).trim();
  //   return url ? { image_url: url } : {};
  // }
}

/***********************
 *  MAIN
 ***********************/
class CameraPageAdapter {
  constructor() {
    // 다양한 HTML 변형을 고려한 셀렉터 후보들
    this.$video = firstSel([
      "#camera-preview",
      "video.camera-preview",
      "video",
    ]);
    this.$placeholder = firstSel([
      "#camera-placeholder",
      ".camera-placeholder",
    ]);
    this.$overlay = firstSel(["#face-overlay", ".face-overlay"]);
    this.$capture = firstSel([
      "#capture-button",
      "#capture-btn",
      ".capture-button",
      "button.capture",
    ]);
    this.$fileInput = firstSel(["#camera-input", "input[type=file]"]);
    this.$genderButtons = qa('[data-option="gender"]');
    this.$ageButtons = qa('[data-option="age"]');

    this.gender = null;
    this.age = null;
    this.stream = null;
    this.isCapturing = false;

    this.bind();
    this.initState();

    // ✅ 추가: signup_info.html에서 저장한 birth_date → age 계산
    const profile = JSON.parse(localStorage.getItem("faceapp_profile") || "{}");
    if (profile.birth_date && !this.age) {
      const birthYear = new Date(profile.birth_date).getFullYear();
      const nowYear = new Date().getFullYear();
      this.age = nowYear - birthYear;
      localStorage.setItem("shoot_age", this.age); // 내부 로직 호환
    }
    if (profile.gender && !this.gender) {
      this.gender = profile.gender;
      localStorage.setItem("shoot_gender", profile.gender);
    }
    this.updateCaptureBtn();
  }

  initState() {
    // 가능하면 이전 선택 복원
    const g = localStorage.getItem("shoot_gender");
    const a = localStorage.getItem("shoot_age");
    if (g) {
      this.gender = g;
      this.markSelected("gender", g);
    }
    if (a) {
      this.age = a;
      this.markSelected("age", a);
    }
    this.updateCaptureBtn();
  }

  bind() {
    // 성별/나이 버튼 (data-option="gender"/"age" 구조 그대로 사용)
    document.addEventListener("click", (e) => {
      const btn = e.target.closest("[data-option][data-value]");
      if (!btn) return;
      const opt = btn.dataset.option;
      const val = btn.dataset.value;

      // 해당 그룹에서 selected 클래스 토글
      const container = btn.parentElement;
      qa("[data-option]", container).forEach((b) =>
        b.classList.remove("selected")
      );
      btn.classList.add("selected");

      if (opt === "gender") {
        this.gender = val;
        localStorage.setItem("shoot_gender", val); // 업로드 시 활용
      } else if (opt === "age") {
        this.age = val;
        localStorage.setItem("shoot_age", val);
      }
      this.updateCaptureBtn();
    });

    // 촬영 버튼
    if (this.$capture) {
      this.$capture.addEventListener("click", () => this.startOrCapture());
    }

    // 파일 업로드 대체 입력(있다면)
    if (this.$fileInput) {
      this.$fileInput.addEventListener("change", (e) =>
        this.handleFileSelect(e)
      );
    }

    // 페이지 숨김 시 카메라 정지
    document.addEventListener("visibilitychange", () => {
      if (document.hidden && this.$video && this.$video.srcObject) {
        this.$video.srcObject.getTracks().forEach((t) => t.stop());
      }
    });
  }

  markSelected(kind, value) {
    const btns = qa(`[data-option="${kind}"]`);
    btns.forEach((b) => {
      if (b.dataset.value == value) b.classList.add("selected");
      else b.classList.remove("selected");
    });
  }

  updateCaptureBtn() {
    if (!this.$capture) return;
    const ok = !!(this.gender && this.age);
    this.$capture.disabled = !ok;
    if (!ok) this.$capture.textContent = "성별/나이대를 선택해주세요";
    else
      this.$capture.textContent =
        this.$video && this.$video.srcObject ? "사진 촬영" : "촬영";
  }

  async startOrCapture() {
    // if (!this.gender || !this.age) {
    //   alert("성별과 나이대를 선택해주세요.");
    //   return;
    // }
    // 이미 카메라 켜져 있으면 촬영
    if (this.$video && this.$video.srcObject) {
      this.capturePhoto();
      return;
    }
    // 카메라 시작
    try {
      this.stream = await navigator.mediaDevices.getUserMedia({
        video: {
          facingMode: "user",
          width: { ideal: 640 },
          height: { ideal: 480 },
        },
        audio: false,
      });
      if (this.$video) {
        this.$video.srcObject = this.stream;
        this.$video.style.display = "block";
      }
      if (this.$placeholder) this.$placeholder.style.display = "none";
      if (this.$overlay) this.$overlay.classList.add("active");
      this.updateCaptureBtn();
    } catch (e) {
      console.error(e);
      alert("카메라 권한을 허용해주세요.");
    }
  }

  async ensureVideoReady(video) {
    // 메타데이터(크기) 준비
    if (video.readyState < 1) {
      await new Promise((res) =>
        video.addEventListener("loadedmetadata", res, { once: true })
      );
    }
    // 첫 프레임 준비
    if (video.readyState < 2) {
      await new Promise((res) =>
        video.addEventListener("loadeddata", res, { once: true })
      );
    }
    // 크기 확정(혹시 0일 때)
    if (!video.videoWidth || !video.videoHeight) {
      await new Promise((res) =>
        video.addEventListener("resize", res, { once: true })
      );
    }
    // 한 프레임 렌더 보장
    await new Promise((res) => requestAnimationFrame(() => res()));
  }

  async capturePhoto() {
    if (!this.$video) return alert("비디오 요소를 찾을 수 없습니다.");
    if (this.isCapturing) return;
    this.isCapturing = true;
    if (this.$capture) {
      this.$capture.disabled = true;
      this.$capture.textContent = "촬영 중...";
    }

    // const canvas = document.createElement("canvas");
    // canvas.width = this.$video.videoWidth;
    // canvas.height = this.$video.videoHeight;
    // const ctx = canvas.getContext("2d");

    // 필요 시 미러 보정(원본이 미러 상태면 주석 해제)
    // ctx.translate(canvas.width, 0); ctx.scale(-1, 1);

    // ctx.drawImage(this.$video, 0, 0, canvas.width, canvas.height);
    // ✅ 비디오 프레임 준비(검은 이미지 방지)
    await this.ensureVideoReady(this.$video);
    await new Promise((r) => requestAnimationFrame(() => r()));

    const vw = this.$video.videoWidth || 640;
    const vh = this.$video.videoHeight || 480;
    if (!vw || !vh) {
      alert("카메라 프레임을 가져오지 못했습니다. 다시 시도해주세요.");
      this.isCapturing = false;
      if (this.$capture) {
        this.$capture.disabled = false;
        this.$capture.textContent = "사진 촬영";
      }
      return;
    }

    // const vw = this.$video.videoWidth || 640;
    // const vh = this.$video.videoHeight || 480;
    const canvas = document.createElement("canvas");
    canvas.width = vw;
    canvas.height = vh;
    const ctx = canvas.getContext("2d");
    // 필요 시 미러 보정 (원본이 좌우반전이면 주석 해제)
    // ctx.translate(canvas.width, 0); ctx.scale(-1, 1);
    ctx.drawImage(this.$video, 0, 0, vw, vh);

    canvas.toBlob(
      // async (blob) => {
      //   try {
      //     // const { image_url } = await uploadCapture(
      //     //   blob,
      //     //   this.gender,
      //     //   this.age
      //     // );
      //     const uploadRes = await uploadCapture(blob, this.gender, this.age);
      //     const image_url =
      //       uploadRes && uploadRes.image_url ? uploadRes.image_url : null;

      //     // 프리뷰 유지(이미지로 대체)
      //     if (this.$video) this.$video.style.display = "none";
      //     if (this.$overlay) this.$overlay.classList.remove("active");
      //     if (this.$placeholder) {
      //       this.$placeholder.innerHTML = "";
      //       this.$placeholder.style.display = "flex";
      //       const img = document.createElement("img");
      //       img.src = image_url;
      //       img.alt = "촬영 이미지";
      //       img.style.width = "100%";
      //       img.style.height = "100%";
      //       img.style.objectFit = "cover";
      //       this.$placeholder.appendChild(img);
      //     }

      //     // 안내(선택)
      //     // alert("업로드 완료!");
      //   } catch (err) {
      //     console.error(err);
      //     alert("업로드에 실패했습니다.");
      //   } finally {
      //     this.isCapturing = false;
      //     if (this.$capture) {
      //       this.$capture.disabled = false;
      //       this.$capture.textContent = "사진 촬영";
      //     }
      //     // 스트림을 바로 끄고 싶으면 아래 주석 해제
      //     // if (this.stream) this.stream.getTracks().forEach(t => t.stop());
      //   }
      // },
      // async (blob) => {
      //   // ✅ 서버 URL이 비어도 결과 페이지에 띄울 수 있도록 base64를 먼저 확보
      //   const base64 = canvas.toDataURL("image/jpeg", 0.85);
      //   try {
      //     const uploadRes = await uploadCapture(blob, this.gender, this.age);
      //     const image_url =
      //       uploadRes && uploadRes.image_url ? uploadRes.image_url : null;

      //     // ✅ 결과 페이지에서 쓸 값 저장
      //     if (image_url) localStorage.setItem("last_capture_url", image_url);
      //     localStorage.setItem("capturedFaceImage", base64);

      //     // 프리뷰 유지(이미지로 대체) — URL 우선, 없으면 base64
      //     if (this.$video) this.$video.style.display = "none";
      //     if (this.$overlay) this.$overlay.classList.remove("active");
      //     if (this.$placeholder) {
      //       this.$placeholder.innerHTML = "";
      //       this.$placeholder.style.display = "flex";
      //       const img = document.createElement("img");
      //       img.src = image_url || base64; // ✅ 안전한 대체
      //       img.alt = "촬영 이미지";
      //       img.style.width = "100%";
      //       img.style.height = "100%";
      //       img.style.objectFit = "cover";
      //       this.$placeholder.appendChild(img);
      //     }

      //     // ✅ 바로 결과 페이지로 이동
      //     location.href = "ai_test_result.html";
      //   } catch (err) {
      //     console.error(err);
      //     alert("업로드에 실패했습니다.");
      //   } finally {
      //     this.isCapturing = false;
      //     if (this.$capture) {
      //       this.$capture.disabled = false;
      //       this.$capture.textContent = "사진 촬영";
      //     }
      //     // 스트림을 바로 끄려면 주석 해제
      //     // if (this.stream) this.stream.getTracks().forEach(t => t.stop());
      //   }
      // },
      async (blob) => {
        const base64 = canvas.toDataURL("image/jpeg", 0.85);
        try {
          const uploadRes = await uploadCapture(blob, this.gender);
          const image_url =
            uploadRes && uploadRes.image_url ? uploadRes.image_url : null;

          if (image_url) localStorage.setItem("last_capture_url", image_url);
          localStorage.setItem("capturedFaceImage", base64);

          // 프리뷰 영역 업데이트(있다면)
          if (this.$video) this.$video.style.display = "none";
          if (this.$overlay) this.$overlay.classList.remove("active");
          if (this.$placeholder) {
            this.$placeholder.innerHTML = "";
            this.$placeholder.style.display = "flex";
            const img = document.createElement("img");
            img.src = image_url || base64;
            img.alt = "촬영 이미지";
            img.style.width = "100%";
            img.style.height = "100%";
            img.style.objectFit = "cover";
            this.$placeholder.appendChild(img);
          }

          // 결과 페이지로 이동
          location.href = "ai_test_result.html";
        } catch (err) {
          console.error(err);
          alert("업로드에 실패했습니다.");
        } finally {
          this.isCapturing = false;
          if (this.$capture) {
            this.$capture.disabled = false;
            this.$capture.textContent = "사진 촬영";
          }
        }
      },
      "image/jpeg",
      0.85
    );
  }

  handleFileSelect(e) {
    const file = e.target.files && e.target.files[0];
    if (!file || !this.$video || !this.$placeholder) return;
    const reader = new FileReader();
    reader.onload = (ev) => {
      // 미리보기 대체 (파일 프리뷰 모드)
      this.$video.style.display = "none";
      this.$placeholder.innerHTML = "";
      this.$placeholder.style.display = "flex";
      const img = document.createElement("img");
      img.src = ev.target.result;
      img.style.width = "100%";
      img.style.height = "100%";
      img.style.objectFit = "cover";
      this.$placeholder.appendChild(img);
    };
    reader.readAsDataURL(file);
  }
}

document.addEventListener("DOMContentLoaded", () => {
  // new CameraPageAdapter(); // 페이지 구동
  if (window.__cameraBooted) return; // ✅ 전역 가드
  window.__cameraBooted = true;
  getSessionId(); // 세션 보장
  const cam = new CameraPageAdapter();
  cam.startOrCapture();
});
